#!/usr/bin/env python3
"""
Blender-MCP Windows Client — runs on Windows alongside Blender.

Polls a remote relay server for commands, forwards them to
local blender-mcp on localhost:9876, posts results back.

Usage:
    python win_client.py --server http://SERVER_IP:8080 --blender localhost:9876
"""
...
class RelayPoller:
    ...
    def poll_once(self):
        try:
            ...
            print(f"  [!] Poll error (after retries): {e}")
            return False
        ...
        try:
            ...
            print(f"  [{cmd_id}] Failed to post response (after retries): {e}")

        return True

    def run(self, interval=1.0):
        print(f"Polling {self.server_url} every {interval}s...")
        print(f"Forwarding to blender-mcp at {self.bridge.host}:{self.bridge.port}")
        print()
...
def main():
    parser = argparse.ArgumentParser(description="Blender-MCP Windows Client")
    parser.add_argument("--server", required=True, help="Relay server URL (http://IP:8080)")
