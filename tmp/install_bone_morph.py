"""把本地 bone_morph 插件推送并启用到远程 Blender。"""
import json
import pathlib
import time
...
LOCAL_ADDON = pathlib.Path(r"C:\Users\Administrator\Desktop\winwork\bone_morph")
ADDON_NAME = "bone_morph"
FILES = [f.name for f in LOCAL_ADDON.iterdir() if f.suffix == '.py']
...
def main():
    ...
    code.append(f"print('INSTALL_OK', target, 'enabled_obj=', repr(ok))")
